...
istprozent <- istprozent[ - nrow(istprozent), -4]

istprozent[ istprozent$FS > 101 , "Aspartam"] <- istprozent[ istprozent$FS > 101 , "FS"]
istprozent[ istprozent$FS > 101 , "Acesulfam"] <- istprozent[ istprozent$FS > 101 , "FS"]
istprozent[ istprozent$FS > 101 , "Flavor"] <- istprozent[ istprozent$FS > 101 , "FS"]


# Model parameter ####
setwd(dt$wd.valset)
dt$model_para <- NULL
para <- names(istprozent)[ - c(1) ]
dt$model_para <- names(istprozent)[ - c(1) ]
...
for(i in 1:length(unique(dt$model_para)))
  mea_s_spc$Probe[ grep( paste0( "_", dt$model_para[i], "_") , mea_s_spc$files_spc) ] <- dt$model_para[i]
...
for(i in 1:length(unique(dt$model_para))){
  ...
  para_prozent <- gsub(" ", "", gsub("_", ",", gsub("%", "", gsub(unique(dt$model_para)[i], "", istprozent[ , 1][ grep( paste0(unique(dt$model_para)[i], " ") , istprozent[ , 1]) ]))))

  para_prozent_flag <- formatC(as.numeric(para_prozent), width = 2, format = "d", flag = "0")

  for(j in 1:length(para_prozent)){

    mea_s_spc$Probe_Anteil[ grep( paste0("_", unique(dt$model_para)[i], "_", para_prozent[j]), mea_s_spc$files_spc)] <- paste0( unique(dt$model_para)[i], "_", para_prozent_flag[j], "%")

    istprozent$Probe_Anteil[ grep(paste(unique(dt$model_para)[i], para_prozent[j], "%"), istprozent$Probe_Anteil) ] <- paste0(unique(dt$model_para)[i], "_", para_prozent_flag[j], "%")
...
# Dilution ####
# Dilution <- substr(istprozentmeaspc$Probe_Anteil, nchar( istprozentmeaspc$Probe_Anteil ) - 3, nchar( istprozentmeaspc$Probe_Anteil ) - 1)
# Dilution <- gsub("_", "", Dilution)
# Dilution <- as.numeric(Dilution)
#
# Dilution[ grep("H2O", istprozentmeaspc$Probe_Anteil) ] <- 100 - Dilution[ grep("H2O", istprozentmeaspc$Probe_Anteil) ]
