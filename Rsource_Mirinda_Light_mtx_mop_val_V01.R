...
dt$para$model.raw.date <- c("220630")
...
dt$para$ylab <- c(bquote("TA in mg / 100mL"^-1), bquote("Aspartam in mg / L"^-1), bquote("Acesulfam in mg / L"^-1), bquote("Topnote in mg / L"^-1), bquote("Flavor in mg / L"^-1))
...
dt$para$SOLL <- c(43.9, 360.3, 85.15, NA, NA)
