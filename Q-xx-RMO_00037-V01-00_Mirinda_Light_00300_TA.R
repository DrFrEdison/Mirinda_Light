...
matplot(dt$para$wl[[1]]
        , t( dt$SL$spc[ grep("Flavor", dt$SL$data$Probe) , ])
...
dt$pls$merge <- lapply(dt$pls$pred, function( x ) merge_pls(pls_pred = x, pls_lm = dt$pls$lm, mean = c(dt$para$SOLL[dt$para$i] * c(.8, 1.2)), R2=.9))
